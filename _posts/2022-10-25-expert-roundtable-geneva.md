---
title: "Sadata and IMPACT OSS presented alongside other digital human rights tracking tools"
subheadline: "Expert Roundtable, 18/10/2022"
teaser: >-
  The Geneva Academy organised an expert roundtable 'De-mystifying digital human rights tracking tools and databases' as part of the '2022 Annual Conference of the Geneva Human Rights Platform', bringing together developers, administrators and users of existing digital tracking tools for human rights monitoring, implementation and follow-up.
lead: >-
  The Geneva Academy organised an expert roundtable 'De-mystifying digital human rights tracking tools and databases' as part of the '2022 Annual Conference of the Geneva Human Rights Platform', bringing together developers, administrators and users of existing digital tracking tools for human rights monitoring, implementation and follow-up.
author: tmfrnz
layout: page-fullwidth
published: true
image_filename: 'expert-roundtable-geneva.png'
image_thumb: 'expert-roundtable-geneva.png'
---

Following a round of short presentations of each tool, participants discussed the common challenges Governments and organisations are facing and explored the potential of technological advances to address them. The identified implementation challenges include lack of coordination across ministries, duplication of activities, and low levels of engagement with the international human rights system at the domestic level. Digital tools were seen as having the potential to address these challenges through clustering of recommendations and obligations, linking obligations to the Sustainable Development Goals and facilitation of coordinated monitoring. Further, the developers agreed on the need for collaboration and technical interoperability.

Alongside tools such as [Universal Human Rights Index (OHCHR)](http://uhri.ohchr.org), [National Recommendations Tracking Database (OHCHR)](https://nrtd.ohchr.org/en), [SIMORE (Paraguay)](https://www.mre.gov.py/simoreplus), [UWAZI (HURIDOCS)](https://uwazi.io), [UPR Info](https://www.upr-info.org/en), [SDG Explorer (Danish Institute for Human Rights)](https://sdgdata.humanrights.dk/en), [Girls Rights Platform (Plan International)](https://database.girlsrightsplatform.org/en), our trustee and technical lead Timo Franz presented the latest “multi-framework” version of [IMPACT OSS](https://impactoss.org/impactoss/) and representatives of Samoa shared their experience with [Sadata](https://sadata.ws), Samoa's national implementation of IMPACT OSS.

**Links**

- Geneva Academy: [geneva-academy.ch](https://geneva-academy.ch)
- 2022 Annual Conference of the Geneva Human Rights Platform: [archives.geneva-academy.ch/event/past-events/detail/349-2022-...](https://archives.geneva-academy.ch/event/past-events/detail/349-2022-annual-conference-of-the-geneva-human-rights-platform.html) [_updated_]
